...
void nbsepleps(){
	...
	for(int m=0;m<4;m++){
		...
		l1->SetBorderSize(0);
		l1->SetNColumns(2);
		l1->AddEntry(lepku.at(m), stcut.at(m) , "l");
		l1->Draw("");
		...
		l2->SetBorderSize(0);
		l2->SetNColumns(2);
		l2->AddEntry(lepuc.at(m), stcut.at(m) , "l");
		l2->Draw("");

		c->Print("./Plots/leps_nb_"+stcut.at(m)+"_comp.pdf");
		c->Print("./Plots/leps_nb_"+stcut.at(m)+"_comp.C");
	}
}
