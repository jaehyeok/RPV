...
h_bin2_b  = TH1D('bin2', 'b-only ttbar norm for bin5',50,0,5);  h_bin2_b.SetCanExtend(1)
...
for i in range(len(binnames)):    
    for j in range(len(binnames[0])):
        #Fill Histograms
        if j not in [14,17,2,5,3,6,12,13]: continue;
        elif j == 14: 
            h_bin2_b.Fill(bvalues[i][14])
            h_bin2_sb.Fill(svalues[i][14])
        elif j == 17: 
            ...
            h_bin14_b.Fill(bvalues[i][5])
            h_bin14_sb.Fill(svalues[i][5])
        elif j == 3: 
            h_bin12_b.Fill(bvalues[i][3])
            h_bin12_sb.Fill(svalues[i][3])
        elif j == 6: 
            h_bin15_b.Fill(bvalues[i][6])
            h_bin15_sb.Fill(svalues[i][6])
        elif j == 12: 
...
h_bin12_b.Draw("hist")
line.DrawLine(pvalues[0][3],0,pvalues[0][3],80)
c1.SaveAs(argv[1]+"/h_bin12_b.pdf")
h_bin14_b.Draw("hist")
line.DrawLine(pvalues[0][5],0,pvalues[0][5],80)
c1.SaveAs(argv[1]+"/h_bin14_b.pdf")
h_bin15_b.Draw("hist")
line.DrawLine(pvalues[0][6],0,pvalues[0][6],80)
