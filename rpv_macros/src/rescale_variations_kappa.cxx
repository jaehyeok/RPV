      ...
      g->cd(binname);
        ...
        if(isBlinded(binNames.at(ibin), blindedBins)) {
            ...
            for(int i=1; i<=data_obs->GetNbinsX(); i++) {
              std::vector<float> qcd_kap1 = {0.871992,0.833822,0.84189};
              std::vector<float> qcd_kap1_err = {0.00568008,0.00534587,0.00822651};
              std::vector<float> qcd_kap2 = {0.85945,0.770475,0.732131};
              std::vector<float> qcd_kap2_err = {0.0148226,0.0148931,0.0194948};
              if(year=="2016"){
                qcd_kap1 = {0.911644,0.856198,0.869877};
                qcd_kap1_err = {0.00861229,0.951178,0.0134983};
           	qcd_kap2 = {0.841984,0.826381,0.784958};
		qcd_kap2_err = {0.0276946,0.026493,0.0328387};
              }
	      float rescaler(1);
	      int njbin = (binnumber-1)%3;
	      if(i==2) rescaler=qcd_kap1[njbin];
	      else if(i==3) rescaler=qcd_kap2[njbin];
	      rescaler=1;
		   ...
		   data_obs->SetBinContent(i, ((qcd->GetBinContent(i)*rescaler)
                            + ttbar->GetBinContent(i)
                            + wjets->GetBinContent(i)
                            + other->GetBinContent(i)));
